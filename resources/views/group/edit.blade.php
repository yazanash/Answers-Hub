...
<div class="container p-3 bg-white">
  ...
  @endif
  <form method="POST" class="col-8" action="{{route('groups.update',$group->id)}}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="mb-3">
      <label for="name" class="form-label">Category name</label>
      <input type="text" class="form-control" value="{{$group->name}}"  id="name" name="name">
    </div>
    <div class="mb-3">
      <label for="description" class="form-label">Description name</label>
      <input type="text" class="form-control" value="{{$group->description}}" id="description" name="description">
    </div>
